...
const handleSubmit = async (e) => {
  e.preventDefault();
  try {
    const res = await axios.post("https://security-dashboard-backend-navy.vercel.app/api/login", {
      email,
      password,
    });
    ...
    if (data.failed_attempts >= 5) {
      toast.error("Your account has been locked due to multiple failed attempts!", {
        position: "top-right",
        autoClose: 2000,
        theme: "colored",
        transition: Bounce,
  ...
  const failedAttempts = err.response?.data?.failed_attempts;
